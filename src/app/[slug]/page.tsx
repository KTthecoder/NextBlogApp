        ...
        <div className="flex w-full flex-col items-center justify-center mt-16 sm:-mt-4 lg:-mt-12">
            <div className="flex flex-col items-center justify-center w-full" style={{maxWidth: 850}}>
                <div className="w-11/12 flex items-center justify-center flex-col border-b-2 border-[#1d1d1d] pb-5 sm:pb-7" >
                    ...
                    <h1 className="text-white text-3xl sm:text-4xl tracking-wider mt-6 sm:mt-4 text-center">{article?.title}</h1>
                    <p style={{backgroundColor: article?.category.color}} className='text-white rounded-lg px-2 py-1 text-sm mt-6'>{article?.category.name}</p>
                </div>
                <div className="w-11/12 flex items-center justify-between mt-5">
                    <div className="flex">
                        <p className='text-white text-base flex flex-row mr-4'><AiOutlineLike className="text-gray-300 text-xl mr-2 sm:text-2xl"/>{article?.likes}</p>
                        <p className='text-white text-base flex flex-row'><FaRegComments className="text-gray-300 text-xl mr-2 sm:text-2xl"/>{article?.Comment.length}</p>
                    </div>
                    <div className="flex">
                        <p className='text-gray-200 tracking-wide flex flex-row items-center'>
                        <AiOutlineCalendar className="text-gray-200 text-xl mr-2 sm:text-2xl"/>{article?.createdAt.toLocaleDateString()}</p>
                    </div>
                </div>
            </div>
